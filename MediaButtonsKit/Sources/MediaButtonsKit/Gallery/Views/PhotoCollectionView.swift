...
struct PhotoCollectionView<Item: PhotoItem>: View {
    ...
    private var imageSize: CGSize {
        return CGSize(width: itemSize.width * min(displayScale, 2), height: itemSize.height * min(displayScale, 2))
    }
    
    func thumbnailSize() -> CGSize {
        CGSize(width: UIScreen.main.bounds.size.width / 3 - itemSpacing, height: UIScreen.main.bounds.size.width / 3 - itemSpacing)
    }
