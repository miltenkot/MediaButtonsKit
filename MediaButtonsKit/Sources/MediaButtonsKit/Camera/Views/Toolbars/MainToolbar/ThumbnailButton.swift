...
public struct ThumbnailButton<CameraModel: Camera>: View {
    ...
    public init(camera: CameraModel, isMultipleSelectionEnabled: Bool = false) {
        self.camera = camera
    ...
    private var photoPicker: some View {
        ...
        if isMultipleSelectionEnabled {
            PhotosPicker(selection: $selectedItems, matching: filter, photoLibrary: photoLibrary) { [thumbnail = camera.thumbnail] in
...
#Preview("Multiple") {
    ThumbnailButton(camera: PreviewCameraModel(), isMultipleSelectionEnabled: true)
}
